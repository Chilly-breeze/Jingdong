<template>
  <div class="buy-page">
    <navgation-bar :pageName="'立即购买'"></navgation-bar>

    <div class="buy-page-content">
      <div class="buy-page-content-goods">
        <img class="buy-page-content-goods-img" :src="goodsData.swiperImgs[0]" />
        <div class="buy-page-content-goods-desc">
          <p class="buy-page-content-goods-desc-name">{{goodsData.name}}</p>
          <p class="buy-page-content-goods-desc-price">￥{{goodsData.price | tofix2}}</p>
        </div>
      </div>
    ...
    <div class="pay">
        <div class="page-commit">立即购买</div>
    </div>
  </div>
</template>
...
export default {
  ...
  created() {
    console.log(this.$route.query,"12223")
    this.getBuy()
  ...
  methods: {
      getBuy(){
        this.goodsData = this.$route.query.goodsDetail;
      },
    /**
  ...
  watch:{
      '$route'(newval){
          this.goodsData = newval.query.goodsDetail
